...
available_vms: List[int] = list(range(50, -1, -1))
active_vms: List[int] = []
...
worker_ports = list(range(20000, 20020))  # 所有 worker 端口
worker_map = {}  # 存储客户端与端口的映射

@app.get("/route")
async def get_worker_port(request: Request):
    """客户端获取分配的固定 Worker 端口"""
    # 从 Cookie 获取或生成客户端 ID
    client_id = request.cookies.get("client_id")
    if not client_id:
        client_id = uuid.uuid4().hex
    
    # 哈希算法分配端口
    port = worker_ports[hash(client_id) % len(worker_ports)]
    worker_map[client_id] = port
    
    # 返回端口并设置 Cookie
    response = responses.JSONResponse(content={"port": port})
    response.set_cookie(key="client_id", value=client_id, max_age=3600)
    return response
        ...
        with vm_lock:
            ...
            for vm_id in active_vms_copy:
                ...
                if not vm_info["visited"]:
                    vm_info["lifetime"] -= 60    # Decrease timeout by 60 seconds
